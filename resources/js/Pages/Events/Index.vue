    ...
    <AuthenticatedLayout>
        <div class="container mx-auto p-4">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Link v-for="event in events" :key="event.id" :href="route('events.show', event.id)" class="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200">
                    <h2 class="text-xl font-semibold mb-2">{{ event.title }}</h2>
                    <p class="text-gray-600">{{ event.description }}</p>
                    <p class="text-gray-600">Location: {{ event.location }}</p>
                    <p class="text-gray-600">Date: {{ event.date }}</p>
                    <p class="text-gray-600" v-if="event.time">Time: {{ event.time }}</p>
                </Link>
            </div>
        </div>
    </AuthenticatedLayout>
